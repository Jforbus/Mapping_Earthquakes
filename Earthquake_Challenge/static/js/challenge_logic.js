...
d3.json(tectonicPlateData).then(function(data) {
  ...
  L.geoJson(data, {
    // style lines with color and weight
    style: {
      color: "#FF00FF",
      weight: 4
    }
...
d3.json(majorEarthquakeData).then(function(data) {
  ...
  //  after the marker has been created and styled.
  L.geoJson(data, {
    // create circleMarker for each earthquake.
    pointToLayer: function(feature, latlng) {
      console.log(data);
      return L.circleMarker(latlng);
    },
    // style using style function
    style: styleInfo,
    // create a popup for each circleMarker for magnitude and location
    onEachFeature: function(feature, layer) {
      layer.bindPopup("Magnitude: " + feature.properties.mag + "<br>Location: " + feature.properties.place);
    }
